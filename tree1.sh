...
# Función para obtener la ruta de configuración de Neovim
get_nvim_config_path() {
  if [ -d "$HOME/.config/nvim" ]; then
    echo "$HOME/.config/nvim"
  else
    echo "$HOME/AppData/Local/nvim" # Para Windows
  fi
}
...
# Escribir encabezado
echo "=== Configuración de Neovim ===" >"$output_file"
echo "Fecha: $(date)" >>"$output_file"
echo "Ruta: $NVIM_CONFIG_PATH" >>"$output_file"
echo "===============================" >>"$output_file"
echo >>"$output_file"

# Encontrar y procesar todos los archivos .lua
find "$NVIM_CONFIG_PATH" -type f -name "*.lua" | while read -r file; do
...
echo "Estadísticas:"
echo "Número de archivos procesados: $(find "$NVIM_CONFIG_PATH" -type f -name "*.lua" | wc -l)"
